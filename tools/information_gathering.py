...
class Gau(HackingTool):
    TITLE = "gau (Get All URLs)"
    DESCRIPTION = (
        "Fetch known URLs from AlienVault OTX, WayBack Machine, and CommonCrawl.\n"
        "Usage: gau example.com | grep '?=' | uro"
    )
    REQUIRES_GO = True
    INSTALL_COMMANDS = [
        "go install github.com/lc/gau/v2/cmd/gau@latest",
    ]
    RUN_COMMANDS = ["gau --help"]
...
class GoSpider(HackingTool):
    ...
    DESCRIPTION = (
        "Fast web spider written in Go — recursive crawling with concurrency.\n"
        "Usage: gospider -s https://example.com -o output"
    )
    REQUIRES_GO = True
    INSTALL_COMMANDS = [
        "go install github.com/jaeles-project/gospider@latest",
    ]
    RUN_COMMANDS = ["gospider --help"]
    PROJECT_URL = "https://github.com/jaeles-project/gospider"


class Naabu(HackingTool):
    TITLE = "Naabu (Fast Port Scanner)"
    DESCRIPTION = (
        "Lightning-fast port scanner by ProjectDiscovery.\n"
        "Usage: naabu -host example.com -p 1-1000"
    )
    REQUIRES_GO = True
    INSTALL_COMMANDS = [
        "go install github.com/projectdiscovery/naabu/v2/cmd/naabu@latest",
    ]
    RUN_COMMANDS = ["naabu --help"]
