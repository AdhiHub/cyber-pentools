...
class WPScan(HackingTool):
    TITLE = "WPScan (WordPress Vulnerability Scanner)"
    DESCRIPTION = (
        "Black-box WordPress vulnerability scanner — detects plugins, themes, weak passwords.\n"
        "Usage: wpscan --url https://example.com --api-token TOKEN"
    )
    SUPPORTED_OS = ["linux", "macos"]
    INSTALL_COMMANDS = ["sudo gem install wpscan || brew install wpscan"]
    RUN_COMMANDS = ["wpscan --help"]
    PROJECT_URL = "https://github.com/wpscanteam/wpscan"
    REQUIRES_RUBY = True


class Joomscan(HackingTool):
    TITLE = "JoomScan (Joomla Vulnerability Scanner)"
    DESCRIPTION = (
        "Joomla CMS vulnerability scanner with component enumeration.\n"
        "Usage: joomscan -u https://example.com"
    )
    SUPPORTED_OS = ["linux"]
    INSTALL_COMMANDS = [
    ...
    RUN_COMMANDS = ["cd joomscan && sudo perl joomscan.pl --help"]
    PROJECT_URL = "https://github.com/OWASP/joomscan"


class Drupwn(HackingTool):
    TITLE = "Drupwn (Drupal Scanner)"
    DESCRIPTION = (
        "Drupal CMS security scanner for version enumeration and vulnerability detection.\n"
        "Usage: drupwn --target https://example.com"
    )
    INSTALL_COMMANDS = ["pip install --user drupwn"]
    RUN_COMMANDS = ["drupwn --help"]
    PROJECT_URL = "https://github.com/immunIT/drupwn"
